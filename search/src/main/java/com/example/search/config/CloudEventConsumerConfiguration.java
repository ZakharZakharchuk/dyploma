...
import com.example.search.endpoint.messaging.consumer.CloudEventConsumer;
...
public class CloudEventConsumerConfiguration {
    ...
    private List<String> qualificationEventTypes;
    ...
    @KafkaListener(topics = "${messaging.events.qualification.receive-topic}",
          groupId = "${messaging.events.qualification.consumer-group-id}")
    public void consumeQualificationEvent(ConsumerRecord<String, String> record) {
        process(record, qualificationEventTypes);
    }
    ...
    protected void loadEventSourcesAndFilters() {
        ...
        this.qualificationEventTypes = asList(
              cloudEventProperties.getQualification().getEventTypes());
    }
}
